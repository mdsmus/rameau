...
(defun unzip (lista)
  "If you think of the list of lists @var{lista} as a matrix, this
transposes it."
  (when (not (null (first lista)))
    (iter (for el in lista)
          (collect (first el) into valor1)
          (collect (rest el) into valor2)
          (finally (return (apply #'values (cons valor1 (unzip valor2))))))))

(defun zip (&rest lists)
  "Builds a list iterating over @var{lists} one element at a time.
The first element in the return value, for example, is a list of all the first elements
in the lists in @var{lists}."
  (iter (until (every #'null lists))
        (collect (mapcar #'first lists))
        (setf lists (mapcar #'rest lists))))
...
(defun normalize-line (zero one min max value)
...
(defun compute-p :private (r* nn P0 total-r total-r*)
  (declare (ignore total-r* total-r))
...
(defun run-lily (result-path result-file png)
  #+sbcl (progn
...
COLLECTION-FORM and subject to QUANTIFIERS. It was taken from
@link{this
webpage}{http://blog.superadditive.com/2007/11/09/list-comprehensions-in-common-lisp}
and probably based on @link{this
paper}{http://www.iro.umontreal.ca/~latendre/publications/listCompFinal.pdf}."
  (labels ((translate (collection-form qs)
             (if (null qs)
                 `(collecting ,collection-form)
                 (translate-generator-or-filter collection-form qs)))
           (translate-generator-or-filter (collection-form qs)
             (let ((q (first qs))
                   (qr (rest qs)))
               (if (eql (first q) (intern "<-" (symbol-package (first q))))
...
(defun cartesian-product (&rest lists)
  (if (rest lists)
      (let ((first (first lists))
            (rest (apply #'cartesian-product (rest lists))))
        (iter (for element in first)
              (nconcing (iter (for r in rest)
                              (collect (cons element r))))))
      (mapcar #'list (first lists))))

(defun average (r)
  "Computes the mean of the elements of @var{r}."
  (/ (iter (for n in r)
           (sum n ))
     (coerce (length r) 'single-float)))

(defun stddev (r)
  "Computes the standard deviation of the elements of @var{r}."
  (let ((a (average r))
        (n (length r)))
    (sqrt (* (/ n (1- n))
             (average (mapcar #L(square (- !1 a)) r))))))

(defmacro-clause (FOR exp IN-PLIST plist)
  `(progn
     (for ,(first exp) in ,plist by #'cddr)
     (for ,(second exp) in (rest ,plist) by #'cddr)))
