...
static int zerov_noteoff = 1; /* If set to 1, this means that a noteon with zero velocity is read as a noteoff */
   ...
   statement. Similarly for "noteoff"... -Davy */
{
	...
	case 0x90:
		if ( Mf_noteon )
		    if (zerov_noteoff==1 && c2==0) {
			(*Mf_noteoff)(chan,c1,c2);
		    }
		    else (*Mf_noteon)(chan,c1,c2);
		break;
	case 0xa0:
		if ( Mf_pressure )
