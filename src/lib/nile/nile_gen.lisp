...
;;;; $Id: nile_gen.lisp,v 1.9 1999/09/28 11:08:44 slink Exp $


;;;; The generational experiment. More or less a copy of
;;;; nile_batali- see there for documentation
;;;; 
;;;; 
;;;; 
;;;; 
...
(eval-when (:compile-toplevel :load-toplevel :execute)
(in-package nile_gen))
(defconstant *RCS-ID* "$Id: nile_gen.lisp,v 1.9 1999/09/28 11:08:44 slink Exp $")
...
(defun batali-round-sd (population population-size
			...
			student-num
			eta)
  (declare (type (integer 2 40000) population-size)
	   (type (integer 0 40000) round)
	   (type (integer 1 40000) meaning-size
		 num-teachers
		 num-meanings)
	   (type type-weight eta)
...
;;;	   (:explain :calls :types :boxing)
  ...
  (let* ((student (svref population student-num))
	 ;; stats
	 ...
	 (st-length 0)			; fixnum
	 (num-episodes (the fixnum (* num-teachers num-meanings)))
	 ;; inner loops
	 (teacher-num 0)
	 (teacher nil)
	 (meaning-num 0)
    ...
    (let ((meaningnum-list (n-random-integers num-meanings meaning-size))
	  (meaningnum-array (make-array (list num-meanings)
					:element-type 'fixnum)))
      (declare (type (cons fixnum) meaningnum-list)
	       (type (simple-array fixnum (*)) meaningnum-array))
      (dotimes (x num-meanings)
	(setf (aref meaningnum-array x) (pop meaningnum-list)))
	...
	(dotimes (y num-meanings)
	  (setq meaning-num (the fixnum (aref meaningnum-array y))
    ...
    ;; now erase teacher
    (kick-net *netspec-batali* teacher :fresh t)
    (format t "Stats: Round ~a teacher ~a Sp-Err ~a He-Err ~a Sp-He-Err ~a Sp-Co ~a He-Co ~a Len ~a~%"
	    round 
	    teacher-num
