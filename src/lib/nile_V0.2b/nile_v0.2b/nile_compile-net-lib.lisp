...
;;;; Unfortunately the above line search functions still seem to yiel bad
;;;; results, so I will try here an exact reimplementation of the functions
;;;; of the netlab package....
...
;;%
;;%	Description
;;%	BR_MIN, BR_MID, BR_MAX, NUM_EVALS] = MINBRACK(F, A, B, FA) finds a
...
;;%	points (in increasing order) such that F(BR_MID) < F(BR_MIN) and
;;%	F(BR_MID) < F(BR_MAX). FA is the value of the function at A: it is
;;%	included to avoid unnecessary function evaluations in the
;;%	optimization routines. The return value NUM_EVALS is the number of
...
;;
(defmacro minbrack (fun, ia, ib, ifa
		    &key values-in	; list in which to store return values)
			 (max-step (coerce 10.0d0 'type-weight)))
  ...
  (flet ((returner (vals)
	   (if values-in
	       (list 'prog2
		     ...
		     nil)
	     (cons 'values vals))))
    (let ((phi (coerce (/ (+ 1.0d0 (sqrt 5.0d0)) 2.0d)) 'type-weight)
	  (tiny (coerce 1.0d-10 'type-weight))
	  a
	  b
	  fa
      ...
      `(let ((,a ,ia)
	     ...
	     (,b ,ib)
	     (,fb (,@fun ,b))
	     (,c ,(coerce 0.0d0 'type-weight))
	     (,fc ,(coerce 0.0d0 'type-act))
	     (,r ,(coerce 0.0d0 'type-weight))
	     (,q ,(coerce 0.0d0 'type-weight))
	     (,u ,(coerce 0.0d0 'type-weight))
	     (,fu ,(coerce 0.0d0 'type-act))
	     (,ulimit ,(coerce 0.0d0 'type-weight))
	     (,bracket_found nil)
	     )
	 (declare (type type-weight ,a ,b ,c ,r ,q ,u ,ulimit)
		  (type type-act ,fa ,fb ,fc ,fu)
		  (boolean ,bracket_found))
	 ;;% Assume that we know going from a to b is downhill initially
	 ;;% (usually because gradf(a) < 0).
	 ;;if (fb > fa)
	 ;;  % Minimum must lie between a and b: do golden section until we find it
	 ;;  c = b;
	 ;;  b = a + (c-a)/phi;
	 ;;  fb = feval(f, b, varargin{:});
	 ;;  num_evals = num_evals + 1;
	 ...
	 (if (> ,fb ,fa)
		 ...
		 ,(returner (list ,c ,b ,a))))
	   ;;else
	   ;;  % Minimum lies at point greater than b
	   ;;  c = b + phi*(b-a);
...
;;;; The pattern iterator macro builds code to do standard functionality
;;;; which involves iterating over patterns, such as to calculate the error
;;;; over all patterns or to do activation/backprop over them.
...
(defmacro trainer (netspec-form net
  ...
  ;; external functions
  (destructuring-bind (&key ((:clear-de-by-dws ext-func-clear-de-by-dws))
       ...
       `((loc-adjust-weights-fn (eta)
				(declare
				 ...
				 (optimize (speed 3) (space 0) (safety 0) (debug 0)
					   (compilation-speed 0))
				 (type type-weight eta))
				(adjust-weights ,netspec-form ,net :eta eta
