...
(defun template-prob (nota)
  (let ((template (rest (nota-pardo-gabarito nota))))
    (length
     (member-if (lambda (x)
                  (equal1 (car x) template))
                *pardo-templates*))))

(defun dim7? (notas)
  (every (lambda (x)
           (equal1 "dim" (stringify (second (nota-pardo-gabarito x)))))
         notas))
...
(defun dim7-res (segmento proximo)
  (let* ((proxima-tonica (nota-pardo-root proximo))
    ...
    (or (find-if
         (lambda (x)
           (equal1 nota (stringify (nota-pardo-root x))))
              segmento)
        (first segmento))))
...
(defun compara-gabarito-pardo-individual (resultado gabarito)
  (let ((nota (note->code (stringify (first resultado))))
    ...
    (and (equal1 nota nota-certa)
         (equal1 (mapcar #'stringify acorde) (mapcar #'stringify acorde-certo)))))
