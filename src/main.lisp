      ...
      (let* ((font (vecto:get-font (rameau-get-font-path "Vera.ttf")))
        ...
        (iter (for (cx cy dx dy cadence size) in boxes)
              ...
              (dbg :cadence-figure "  Painting cadence ~a at center ~a ~a size ~a~%" i
                      ...
                      (+ cy (second center))
                      size)
              (for red = (random-color))
              (for green = (random-color))
              (for blue = (random-color))
              (vecto:set-rgb-stroke red green blue)
              (vecto:set-rgb-fill red green blue)
              (vecto:set-font font size)
              (vecto:centered-string-paths (+ cx (first center))
                                           (+ cy (second center))
                                           cadence)
              ;(vecto:rounded-rectangle (- (+ (first center) (- cx dx)) 5)
              ;                         (+ (+ (second center) (- cy dy)) 5)
              ;                         (* 2 dx)
              ;                         (+ (* 2 dy) 2)
              ;                         1
              ;                         1)
