...
(defun make-cadence-figure (cadences name)
  ...
  (cl-cairo2:with-png-file ((format nil "~a/analysis/cadences-~a.png" *rameau-path* name) 'cl-cairo2:format-rgb24 2000 2000)
    (let* ((center (list 1000 1000))
      ...
      (cl-cairo2:select-font-face "Bistream Vera Sans Mono" 'cl-cairo2:font-slant-normal 'cl-cairo2:font-weight-normal)
              ...
              (iter (for n from 1 to 220)
                    (if boxes
                        (if (some #'identity (mapcar #L(collides !1 box) boxes))
                            (return)
                            (setf box (cons (- (first box) stepx) (cons (- (second box) stepy)
                                                                        dim))))
                        (progn (setf box (cons 0 (cons 0 dim))) (return))))
              (push box boxes)))
      (iter (for (cx cy dx dy cadence size) in boxes)
            (for i from 1)
            (cairo-random-stroke-fill-colors)
            (cl-cairo2:set-font-size size)
            (multiple-value-bind (xbear ybear width height) (cl-cairo2:text-extents cadence)
              (cl-cairo2:move-to (+ (first center) (+ (- cx width) xbear))
                                 (+ (second center) (+ (- cy height) ybear))))
                  ...
                  (let ((font (vecto:get-font (rameau-get-font-path "Vera.ttf"))))
                    ...
                    (iter (for cur-h from (- height 40) above 0 by 20)
                          ...
                          (iter (for pitch in vec)
                                ...
                                (for p from 0)
                                (unless (= 0 pitch)
                                  (vecto:set-font font (max 5 (min 15 (* 35 pitch))))
                                  (aif (gethash p pitch-colors)
                                       (set-stroke-fill-colors it)
                                       (setf (gethash p pitch-colors) (rameau::random-stroke-fill-colors)))
                                  (vecto:string-paths (+ 20 i) cur-h (print-note (code->notename p)))
                                  (vecto:fill-and-stroke)))
                          (vecto:set-font font 20)
                          (aif (gethash (equivalent-pitch re) pitch-colors)
                               (set-stroke-fill-colors it)
                               (setf (gethash (equivalent-pitch re) pitch-colors) (rameau::random-stroke-fill-colors)))
