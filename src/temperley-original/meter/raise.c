...
double best_raising_choice(int base_beat, int back, int *choice, int use_higher_base) {
  ...
  if (base_beat - back < 0) {
    ...
    /* The 0.5 here is a bonus if the first beat at level 4 coincides with 
       the first beat at level 3 */
    *choice = -1;
    return base;  
  }
...
void compute_higher_level_scores(int base_level, int current_level) {
  ...
  for (beat=0; beat < N_bl; beat++) {
    for (back = 2; back <= 3; back++) {
	    bl_array[beat].score[back-2] = best_raising_choice(beat, 
                                                         back, 
                                                         &choice, 
...
void compute_higher_level(int base_level) {
  ...
  if (new_level >= N_LEVELS) {
    fprintf(stderr, 
            "%s: attempting to compute a level that's out of range.\n", 
            this_program);
    exit(1);
  }
