...
(defun estimate-transitions (chords)
  (let ((chords (mapcar #'label->number (mapcan #'identity chords)))
        (transitions (make-array (list *nlabels* *nlabels*) :initial-element 0d0 :element-type 'double-float)))
    ...
    (iter (for c from 0 below *nlabels*)
          (let ((sum (iter (for j from 0 below *nlabels*) (sum (aref transitions c j))))
                (zeros (iter (for j from 0 below *nlabels*) (counting (= 0 (aref transitions c j))))))
            (iter (for j from 0 below *nlabels*)
                  (setf (aref transitions c j) (log (/ (not-zero (aref transitions c j) (/ 1 (+ 0.000001 zeros))) (1+ sum))))
                  )))
    transitions))
          ...
          (iter (for j from 0 below *nlabels*)
                (let ((prob (probabilities seg notes specials j)))
                  (multiple-value-bind (maxjp maxp)
                      (argmax  #L(+ (aref tp (1- i) !1) (aref tran !1 j)) 0 *nlabels*)
...
(defun bayes-decode (segments alg)
  (let ((notes (notes alg))
    ...
    (iter (for s in segments)
          ...
          (collect (number->label (argmax #L(probabilities s notes specials !1) 0 *nlabels*))))))
