  ...
  (let ((notes (notes alg)))
    (with-canvas (:width  800 :height 1050)
      (let ((font (get-font (concat *rameau-path* "fonts/" "Vera.ttf"))))
        ...
        (set-font font 10)
        (format t "Putting the notenames~%")
        (iter (for i from 0 to 95)
              (string-paths 5 (+ 5 (* i 10)) (print-note (code->notename i)))
              (stroke))
        (format t "Putting the chords~%")
        (iter (for i from 0)
        ...
        (let ((max most-negative-double-float)
          ...
          (format t "Min:~a Max:~a~% Drawing the probabilities..;~%"
                  min max)
  ...
  (let ((tr (trans alg)))
    (with-canvas (:width  8000 :height 500)
      (let ((font (get-font "/usr/share/fonts/truetype/ttf-bitstream-vera/Vera.ttf")))
        ...
        (set-rgb-stroke 1 1 1)
        (set-font font 10)
        (format t "Putting the input labels~%")
        (iter (for i from 0)
              (for c in (last *labels* 40))
              (string-paths 0 (+ 5 (* i 10)) (format nil "~a" c))
              (stroke))
        (format t "Putting the output labels~%")
        (iter (for i from 0)
              (for c in *labels*)
              (string-paths (+ 40 (* 3 i)) (+ 410 (mod (* 10 i) 100)) (format nil "~a" c))
              (stroke))
        (format t "Finding the minimal and maximal weights~%")
        (set-rgb-fill 1 0 0)
        (let ((max most-negative-double-float)
              (min most-positive-double-float))
