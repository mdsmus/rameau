  ...
  (with-canvas (:width 2000 :height 2000)
    (let ((nn (prepare (knn-nn alg)))
      ...
      (destructuring-bind (first-count first-chord first-vector) (first nn)
        ...
        (set-font font (normalize 1 55 0 (log first-count) (log first-count)))
        (centered-string-paths (first center) (second center) first-chord)
        (iter (for (count chord vector) in (rest nn))
